...
class FigmaMCPServer {
  ...
  private encodeNodeId(nodeId: string): string {
  ...
  private setupToolHandlers(): void {
      ...
      return {
        tools: [
          ...
          },
          {
            ...
            description: 'Get a detailed analysis of a specific Figma node with all properties (perfect for Cursor)',
            inputSchema: {
              type: 'object',
              properties: {
                fileKey: {
    ...
    this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
      ...
      try {
        switch (name) {
          case 'get_figma_file':
            const fileArgs = args as { fileKey: string };
            return await this.getFigmaFile(fileArgs.fileKey);

          case 'get_figma_file_nodes':
            const nodeArgs = args as { fileKey: string; nodeIds: string[] };
  ...
  private async getFigmaFileNodes(fileKey: string, nodeIds: string[]) {
    try {
      ...
      if (axios.isAxiosError(error)) {
        throw new McpError(
          ErrorCode.InternalError,
          `Figma API error: ${error.response?.status} ${error.response?.statusText}. Response: ${JSON.stringify(error.response?.data)}`
        );
      }
      throw error;
    }
  ...
  private async getFigmaNodeSummary(fileKey: string, nodeId: string) {
    try {
      // Normalize and encode the node ID
      const normalizedNodeId = this.encodeNodeId(nodeId);
      
      console.error(`Fetching node summary: ${normalizedNodeId} for file: ${fileKey}`);
      
      const response = await axios.get(
        `https://api.figma.com/v1/files/${fileKey}/nodes?ids=${normalizedNodeId}`,
        {
          headers: {
            'X-Figma-Token': FIGMA_TOKEN,
          },
      ...
      if (!nodeData) {
        throw new McpError(
          ErrorCode.InvalidParams, 
          `Node ${nodeId} (normalized: ${originalNodeId}) not found. Available nodes: ${Object.keys(response.data.nodes).join(', ')}`
        );
      }
      ...
      // Helper function to format fills
      const formatFills = (fills: any[]) => {
        if (!fills || fills.length === 0) return 'none';
        return fills.map((fill, i) => {
          let fillStr = `  ${i + 1}. Type: ${fill.type}`;
          if (fill.color) fillStr += `, Color: ${formatColor(fill.color)}`;
          if (fill.opacity !== undefined) fillStr += `, Opacity: ${fill.opacity}`;
          if (fill.blendMode) fillStr += `, Blend: ${fill.blendMode}`;
          return fillStr;
        }).join('\n');
      };

      // Helper function to format strokes
      const formatStrokes = (strokes: any[]) => {
        if (!strokes || strokes.length === 0) return 'none';
        return strokes.map((stroke, i) => {
          let strokeStr = `  ${i + 1}. Type: ${stroke.type}`;
          if (stroke.color) strokeStr += `, Color: ${formatColor(stroke.color)}`;
          if (stroke.opacity !== undefined) strokeStr += `, Opacity: ${stroke.opacity}`;
          return strokeStr;
        }).join('\n');
      };
      ...
      const formatEffects = (effects: any[]) => {
        ...
        return effects.map((effect, i) => {
          let effectStr = `  ${i + 1}. ${effect.type}`;
          if (effect.color) effectStr += `, Color: ${formatColor(effect.color)}`;
          if (effect.radius !== undefined) effectStr += `, Radius: ${effect.radius}`;
          if (effect.offset) effectStr += `, Offset: (${effect.offset.x}, ${effect.offset.y})`;
          return effectStr;
        }).join('\n');
      };

      // Helper function to format children summary
      const formatChildren = (children: any[]) => {
        if (!children || children.length === 0) return 'none';
        return children.slice(0, 5).map((child, i) => {
          return `  ${i + 1}. ${child.name || 'Unnamed'} (${child.type})`;
        }).join('\n') + (children.length > 5 ? `\n  ...and ${children.length - 5} more` : '');
      };

      // Build comprehensive summary
      let summaryText = `🎨 **Detailed Figma Node Analysis for ${nodeId}**\n\n`;
      
      // Basic Info
      summaryText += `## 📝 Basic Information\n`;
      summaryText += `**Name:** ${node.name || 'Unnamed'}\n`;
      summaryText += `**Type:** ${node.type}\n`;
      summaryText += `**Visible:** ${node.visible !== false ? 'Yes ✅' : 'No ❌'}\n`;
      
      // Dimensions & Position
      if (node.absoluteBoundingBox) {
        const box = node.absoluteBoundingBox;
        summaryText += `**Dimensions:** ${Math.round(box.width)}px × ${Math.round(box.height)}px\n`;
        summaryText += `**Position:** x: ${Math.round(box.x)}, y: ${Math.round(box.y)}\n`;
      }

      // Opacity & Blend Mode
      if (node.opacity !== undefined) {
        summaryText += `**Opacity:** ${Math.round(node.opacity * 100)}%\n`;
      }
      if (node.blendMode) {
        summaryText += `**Blend Mode:** ${node.blendMode}\n`;
      }

      // Layout Properties (for frames/groups)
      if (node.layoutMode) {
        summaryText += `\n## 📐 Layout\n`;
        summaryText += `**Layout Mode:** ${node.layoutMode}\n`;
        if (node.primaryAxisSizingMode) summaryText += `**Primary Axis:** ${node.primaryAxisSizingMode}\n`;
        if (node.counterAxisSizingMode) summaryText += `**Counter Axis:** ${node.counterAxisSizingMode}\n`;
        if (node.primaryAxisAlignItems) summaryText += `**Align Items:** ${node.primaryAxisAlignItems}\n`;
        if (node.counterAxisAlignItems) summaryText += `**Justify Content:** ${node.counterAxisAlignItems}\n`;
        if (node.itemSpacing !== undefined) summaryText += `**Gap:** ${node.itemSpacing}px\n`;
        if (node.paddingLeft !== undefined) {
          summaryText += `**Padding:** ${node.paddingTop || 0}px ${node.paddingRight || 0}px ${node.paddingBottom || 0}px ${node.paddingLeft || 0}px\n`;
        }
      }

      // Corner Radius
      if (node.cornerRadius !== undefined) {
        summaryText += `**Corner Radius:** ${node.cornerRadius}px\n`;
      }
      ...
      if (node.type === 'TEXT') {
        summaryText += `\n## 📝 Typography\n`;
        if (node.characters) {
          summaryText += `**Text:** "${node.characters.length > 100 ? node.characters.substring(0, 100) + '...' : node.characters}"\n`;
        }
        if (node.style) {
          const style = node.style;
          if (style.fontFamily) summaryText += `**Font:** ${style.fontFamily}\n`;
          if (style.fontSize) summaryText += `**Size:** ${style.fontSize}px\n`;
          if (style.fontWeight) summaryText += `**Weight:** ${style.fontWeight}\n`;
          if (style.lineHeightPx) summaryText += `**Line Height:** ${style.lineHeightPx}px\n`;
          if (style.letterSpacing) summaryText += `**Letter Spacing:** ${style.letterSpacing}px\n`;
          if (style.textAlignHorizontal) summaryText += `**Align:** ${style.textAlignHorizontal}\n`;
        }
      }

      // Fills
      if (node.fills && node.fills.length > 0) {
        summaryText += `\n## 🎨 Fills\n${formatFills(node.fills)}\n`;
      }

      // Strokes
      if (node.strokes && node.strokes.length > 0) {
        summaryText += `\n## 🖊️ Strokes\n`;
        summaryText += `${formatStrokes(node.strokes)}\n`;
        if (node.strokeWeight) summaryText += `**Stroke Weight:** ${node.strokeWeight}px\n`;
        if (node.strokeAlign) summaryText += `**Stroke Align:** ${node.strokeAlign}\n`;
      }

      // Effects
      if (node.effects && node.effects.length > 0) {
        summaryText += `\n## ✨ Effects\n${formatEffects(node.effects)}\n`;
      }

      // Children
      if (node.children && node.children.length > 0) {
        summaryText += `\n## 👶 Children (${node.children.length} total)\n${formatChildren(node.children)}\n`;
      }

      // Constraints
      if (node.constraints) {
        summaryText += `\n## 📍 Constraints\n`;
        summaryText += `**Horizontal:** ${node.constraints.horizontal || 'none'}\n`;
        summaryText += `**Vertical:** ${node.constraints.vertical || 'none'}\n`;
      }
  ...
  private async getFigmaFileOverview(fileKey: string) {
    try {
      const response = await axios.get(`https://api.figma.com/v1/files/${fileKey}`, {
        headers: {
          'X-Figma-Token': FIGMA_TOKEN,
        ...
        document.children.forEach((page: any) => {
          if (page.children) {
            page.children.forEach((child: any) => {
              if (child.type === 'FRAME') totalFrames++;
              if (child.type === 'COMPONENT') totalComponents++;
            });
          }
        });
      }
        ...
        content: [
          {
            ...
            text: `📄 **Figma File Overview**\n\n` +
                  ...
                  `**Pages:**\n` +
                  (document.children ? 
                    document.children.map((page: any, i: number) => 
                      `  ${i + 1}. ${page.name || 'Unnamed Page'} (${page.children ? page.children.length : 0} items)`
                    ).join('\n') : 
                    'No pages found'),
          },
        ],
  ...
  private async generateCodeFromFigmaNode(
    fileKey: string, 
    nodeId: string, 
    ...
    try {
      ...
      const generateCSS = (node: any) => {
        ...
        if (node.type === 'TEXT' && node.style) {
          const style = node.style;
          if (style.fontFamily) styles.fontFamily = `'${style.fontFamily}', sans-serif`;
          if (style.fontSize) styles.fontSize = `${style.fontSize}px`;
          if (style.fontWeight) styles.fontWeight = style.fontWeight;
          if (style.lineHeightPx) styles.lineHeight = `${style.lineHeightPx}px`;
          if (style.letterSpacing) styles.letterSpacing = `${style.letterSpacing}px`;
...
if (node.layoutMode) {
    ...
    if (node.primaryAxisAlignItems) {
      ...
      else if (alignment === 'MAX') styles.justifyContent = 'flex-end';
      else styles.justifyContent = 'flex-start';
    }

    if (node.counterAxisAlignItems) {
      const alignment = node.counterAxisAlignItems;
      if (alignment === 'CENTER') styles.alignItems = 'center';
      else if (alignment === 'MAX') styles.alignItems = 'flex-end';
...
const stylesToReactStyle = (styles: any) => {
...
const generateChildren = (node: any): string => {
  if (!node.children || node.children.length === 0) {
...
export default ${name};`;
  ...
  case 'vue':
    ...
    const generateVueChildCSS = (node: any, prefix: string = ''): string => {
      let css = '';
      if (node.children) {
...
${stylesToCSS(vueStyles)}
  ...
  case 'html':
    ...
    const generateChildCSS = (node: any, prefix: string = ''): string => {
      let css = '';
      if (node.children) {
        node.children.forEach((child: any) => {
          const childClass = child.name?.replace(/[^a-zA-Z0-9]/g, '').toLowerCase() || 'element';
